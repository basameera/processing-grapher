...
void setup() {
  ...
  myChart = cp5.addChart("dataflow")
    ...
    .setStrokeWeight(1.5).setResolution(100)
    .setColorCaptionLabel(color(40))
    ;

  myChart.addDataSet("incoming");
  myChart.setData("incoming", new float[300]);

  myChart.addDataSet("sig");
  myChart.setData("sig", new float[300]);
  
  myChart.setCaptionLabel("abc");
  myChart.setValueLabel("absfsfasdfc");
...
void draw() {
  ...
  sig = sin(frameCount*0.1)*10;
  // push: add data from right to left (last in)
  myChart.push("incoming", sig);
  myChart.push("sig", (-sig*0.6));
}
